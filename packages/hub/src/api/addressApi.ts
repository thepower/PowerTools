import { Maybe, MaybeUndef } from '../typings/common';
import { AddressScopeEnum, AddressType } from './typings';

let globalCrcTable: Maybe<number[]> = null;

const makeCRCTable = (): number[] => {
  let crcTable: Maybe<number[]> = globalCrcTable;
...
const crc32 = (address: Uint8Array): number => {
...
const decodeAddress = (address: AddressType): Uint8Array => {
...
const pad = (base: number, num: number, size: number): string => {
  let result;
  ...
  if (base === 26) {
    ...
    result = String.fromCharCode(B1 + A) + String.fromCharCode(B0 + A);
  } else {
...
export const AddressApi = {
  encodeAddress(address: Uint8Array) {
    ...
    if (address[0] & 0x20) { //private
      ...
      return {
        scope: AddressScopeEnum.private,
      ...
      const txt = pad(26, parseInt((group / 100).toString()), 2) + pad(10, group % 100, 2) + pad(10, IntPart, 14) + pad(10, checksum % 100, 2);
      return {
        scope: AddressScopeEnum.private,
  ...
  parseTextAddress(textAddress: string): Uint8Array {
  ...
  hexToAddress(hex: string): MaybeUndef<number[]> {
    return hex.match(/(.{1,2})/g)?.map(item => parseInt(item, 16));
  },
  ...
  hexToTextAddress(hexAddress: string) {
    return this.encodeAddress(this.hexToAddress(hexAddress) as any)?.txt
  }
};
